...
class PayrollController extends Controller
{
    ...
    public function runPreview(Request $request): JsonResponse
    {
        ...
        foreach ($employeeUserIds as $employeeUserId) {
            ...
            $bank = $this->bankDetailsForPayslip($m, $empUser);
            $row['bankName'] = $bank['bank_name'];
            $row['bankAccountNumber'] = $bank['bank_account_number'];
            $row['bankIfsc'] = $bank['bank_ifsc'];
            $row['bankAccountHolderName'] = $empUser->bank_account_holder_name ?? $m->name ?? $empUser->name;
    ...
    private function executePayrollRun(
        ...
        $this->draftService->markFinalized(
            (string) $user->company_id,
            $year,
            $month,
            (string) $period->id,
    ...
    private function mapSavedPayslipToPreviewRow(
        ...
        $master = $user
            ? $this->currentMasterForEmployee((string) ($user->company_id ?? ''), (string) $payslip->employee_user_id)
            : null;
        $bank = $user
            ? $this->bankDetailsForPayslip($master, $user)
            : [
                'bank_name' => $payslip->bank_name,
                'bank_account_number' => $payslip->bank_account_number,
                'bank_ifsc' => $payslip->bank_ifsc,
            ];
        ...
        return [
            ...
            'bankName' => $this->firstNonEmptyString($payslip->bank_name, $bank['bank_name']),
            'bankAccountNumber' => $this->firstNonEmptyString(
