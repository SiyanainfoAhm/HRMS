...
class PayrollAmendmentService
{
    ...
    public const FIELD_LABELS = [
        ...
        'electricityUnitsConsumed' => 'Electricity Units',
        'water' => 'Water',
        'mess' => 'Mess',
    ...
    public function monthlyAttributesFromPreview(HrmsGovernmentMonthlyPayroll $existing, array $row): array
    {
        ...
        return [
            ...
            'electricity_tariff_id' => $gm['electricityTariffId'] ?? $gm['electricity_tariff_id'] ?? $existing->electricity_tariff_id ?? null,
            'electricity_sthir_aakar' => $num($gm['electricitySthirAakar'] ?? $gm['electricity_sthir_aakar'] ?? $existing->electricity_sthir_aakar ?? 0),
            'electricity_consumption_charge' => $num($gm['electricityConsumptionCharge'] ?? $gm['electricity_consumption_charge'] ?? $existing->electricity_consumption_charge ?? 0),
            'electricity_vahan_aakar' => $num($gm['electricityVahanAakar'] ?? $gm['electricity_vahan_aakar'] ?? $existing->electricity_vahan_aakar ?? 0),
            'electricity_fuel_charge' => $num($gm['electricityFuelCharge'] ?? $gm['electricity_fuel_charge'] ?? $existing->electricity_fuel_charge ?? 0),
            'electricity_duty' => $num($gm['electricityDuty'] ?? $gm['electricity_duty'] ?? $existing->electricity_duty ?? 0),
            'electricity_total' => $num($gm['electricityTotal'] ?? $gm['electricity_total'] ?? $ded['electricity'] ?? $existing->electricity_total ?? 0),
            'electricity_applicable' => (bool) ($gm['electricityApplicable'] ?? $gm['electricity_applicable'] ?? $existing->electricity_applicable ?? true),
            'electricity_mode' => (string) ($gm['electricityMode'] ?? $gm['electricity_mode'] ?? $existing->electricity_mode ?? 'unit_based'),
