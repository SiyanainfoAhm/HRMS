...
function transformKeysToSnake(obj: any, preserveNested = false): any {
  ...
  if (obj !== null && typeof obj === "object" && !(obj instanceof Date)) {
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (preserveNested || PRESERVE_NESTED_CASING_KEYS.has(key) || PRESERVE_NESTED_CASING_KEYS.has(snakeKey)) {
        // Keep nested snapshot structure as sent by the UI (camelCase).
        out[snakeKey] = value;
        continue;
      }
      out[snakeKey] = transformKeysToSnake(value, false);
    }
    return out;
  }
  return obj;
