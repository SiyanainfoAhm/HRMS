...
export async function GET() {
  ...
  if (!companyId) return NextResponse.json({ designations: [] });
...
export async function POST(request: NextRequest) {
  ...
  const level = body?.level === "" || body?.level === null || body?.level === undefined ? null : Number(body.level);
  if (!title) return NextResponse.json({ error: "Title is required" }, { status: 400 });

  const { data, error } = await supabase
    .from("HRMS_designations")
    .insert([{ company_id: companyId, title, level }])
    .select("*")
    .single();
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });
...
export async function PUT(request: NextRequest) {
  ...
  const payload: Record<string, any> = {
    ...
    level: body?.level === undefined ? undefined : body.level === "" ? null : Number(body.level),
  };
...
export async function DELETE(request: NextRequest) {
  ...
  const { error } = await supabase.from("HRMS_designations").delete().eq("id", id).eq("company_id", companyId);
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });
