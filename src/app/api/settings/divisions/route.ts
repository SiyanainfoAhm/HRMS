...
export async function GET() {
  ...
  if (!companyId) return NextResponse.json({ divisions: [] });
...
export async function PUT(request: NextRequest) {
  ...
  if (!id) return NextResponse.json({ error: "Division id is required" }, { status: 400 });
...
export async function DELETE(request: NextRequest) {
  ...
  const { error } = await supabase.from("HRMS_divisions").delete().eq("id", id).eq("company_id", companyId);
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });
